...
(define (random-pos-between-points pos1 pos2)
  (let ((angle (atan (- (pos2 'get-y) (pos1 'get-y)) (- (pos2 'get-x) (pos1 'get-x))))
    ...
    (if (= (round (cos angle)) 0)
        (let ((calculated-y '()))
          (if (< (pos1 'get-y) (pos2 'get-y))
              (set! calculated-y (random (inexact->exact (+ (pos1 'get-y) 1)) (inexact->exact (pos2 'get-y))))
              (set! calculated-y (random (inexact->exact (+ (pos2 'get-y) 1)) (inexact->exact (pos1 'get-y)))))
          (set! calculated-position (make-position (pos1 'get-x) calculated-y)))
        (let ((calculated-x '()))
          (if (< (pos1 'get-x) (pos2 'get-x))
              (set! calculated-x (random (inexact->exact (+ (pos1 'get-x) 1)) (inexact->exact (pos2 'get-x))))
              (set! calculated-x (random (inexact->exact (+ (pos2 'get-x) 1)) (inexact->exact (pos1 'get-x)))))
          (set! calculated-position (make-position calculated-x (pos1 'get-y)))))
    calculated-position))
