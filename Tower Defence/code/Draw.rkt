  ...
  (let* ((window (make-window width height "Tower Defense" 60))
    ...
    (define (draw-game-status-text)
      ...
      (let ((text-tile (make-tile width height)))
        ((text-tile 'draw-text!) (string-append "Points: " (number->string (player 'get-points))) (* size-factor 15) (* size-factor 650) (* size-factor 50) "black")
...
;        (draw-stripes-vertical!)
  ...
  (define (draw-menu!)
    ...
    (let ((tile (make-tile width height))
      ...
      ((tower-text 'draw-text!) "Towers" (* size-factor 20) (* size-factor 650) (* size-factor 120) "white")
