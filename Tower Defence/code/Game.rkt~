...
(define (make-game environment wave player draw)
  ...
  (let ((monster-spawn-time 0)
    ...
    (define (clean!)
      (((draw 'get-text-layer) 'remove-drawable!) ((environment 'draw) 'get-start-text))
      (load-world!)
      ...
      (((draw 'get-window) 'set-mouse-click-callback!) ;Reacts and acts upon mouse clicks
       (lambda (button status x y)
         (if (and (eq? button 'left)
                  (eq? status 'pressed))
             (let ((tower (make-tower 1 (make-position (- x (- (modulo x (ceiling (* size-factor 50))) (- (ceiling (* size-factor 50)) (* size-factor 50)))) (- y (- (modulo y (ceiling (* size-factor 50))) (- (ceiling (* size-factor 50)) (* size-factor 50))))) environment)))
