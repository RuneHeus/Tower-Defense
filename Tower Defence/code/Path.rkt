...
(define (make-path start-position end-position)
  ...
  (let* ((start start-position)
         ...
         (path4 path-4)
         (end end-position)
         (path-positions (list start path1 path2 path3 path4 end)))
    ...
    (define (on-path? object)
      (define (iter lijst)
        (if (not (null? (cdr lijst)))
            (let* ((object-pos (object 'get-position))
                   (next-pos (next-path-to-pos (car lijst)))
                   (max-x (max (next-pos 'get-x) ((car lijst) 'get-x)))
                   (min-x (min (next-pos 'get-x) ((car lijst) 'get-x)))
                   (max-y (max (next-pos 'get-y) ((car lijst) 'get-y)))
                   (min-y (min (next-pos 'get-y) ((car lijst) 'get-y))))
          
              (if (and (and (<= min-x (object-pos 'get-x))
                            (<= (object-pos 'get-x) max-x))
                       (and (<= min-y (object-pos 'get-y))
                            (<= (object-pos 'get-y) max-y)))
                  #t
                  (iter (cdr lijst))))
            #f))
      (iter path-positions))
